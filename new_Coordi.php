<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $sql_1 = "select * from Coordi";
      $result_1 = mysqli_query($conn,$sql_1);
      $count_1 = $result_1->num_rows;

      $sql = "insert into Coordi (Name,Uname,Password,Email) values ('$myname','$myusername','$mypassword','$myemail')";//SELECT SID FROM Student WHERE SID = '$myusername' and Password = '$mypassword'
      $result = mysqli_query($conn,$sql);                                                                                                                   //$result=$conn->query($sql);                                                                                              //$result = mysqli_query($db,$sql);
                                                                                            //$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
                                                                                             //$active = $row['active'];

      $sql_2 = "select * from Coordi";
      $result_2 = mysqli_query($conn,$sql_2);
      $count_2 = $result_2->num_rows;

      if($count_2 == ($count_1 + 1)) {

         echo "New Coordi created";
         header("refresh:3; url = welcome_new2_Coordi.php");
      }
      else {
         echo" ! Oops !, seems you made a mistake ";
         header("location: create_new_Coordi.html");
